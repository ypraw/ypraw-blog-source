...
const processFile = (file, jobs, cb, reporter) => {
  ...
  function () {
    var _ref = (0, _asyncToGenerator2.default)(function* (job) {
      ...
      if (job.file.extension === `png` && args.toFormat === `` || args.toFormat === `png`) {
        clonedPipeline.toBuffer().then(sharpBuffer => imagemin.buffer(sharpBuffer, {
          plugins: [imageminPngquant({
            quality: `${args.quality}-${Math.min(args.quality + 25, 100)}`,
            // e.g. 40-65
      ...
      } else if (pluginOptions.useMozJpeg && (job.file.extension === `jpg` && args.toFormat === `` || job.file.extension === `jpeg` && args.toFormat === `` || args.toFormat === `jpg`)) {
        clonedPipeline.toBuffer().then(sharpBuffer => imagemin.buffer(sharpBuffer, {
...
function queueImageResizing({
  ...
  queueJob(job, reporter); // encode the file name for URL
  ...
  const prefixedSrc = options.pathPrefix + `/static/${digestDirPrefix}` + encodedImgSrc;
  return {
    src: prefixedSrc,
